...
export class SendEmail {
    constructor() { }
    async sentEmailVerification(name, email, verificationCode) {
        ...
        const sentVerificationEmail = async (name, toEmail, verificationCode) => {
            const mailOptions = {
...
<p>Thanks,<br>The SkillHub Team</p>`,
    ...
    async sentResetLinkVerification(name, email, resetToken) {
        ...
        const sentResetLinkEmail = async (name, email, resetToken) => {
            try {
                const mailOptions = {
                    from: process.env.MAILER_EMAIL,
                    to: email,
