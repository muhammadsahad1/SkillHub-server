...
export function adminRoute(route: Route): Route {
  ...
  route.get("/users", (req: Req, res: Res, next: Next) => adminController.getUsers(req, res, next));
  ...
  route.get('/dashboard', (req: Req, res: Res, next: Next) => adminController.dasboardData(req, res, next));
